...
// ==================================================================
// CLUBS.PHP QUERIES (NEWLY ADDED)
// ==================================================================

// All clubs list (for clubs.php?club_id=0)
function getAllClubs(): array {
    global $pdo;
    return $pdo->query("SELECT * FROM clubs ORDER BY name")->fetchAll(PDO::FETCH_ASSOC);
}
...
// Full league standings with GF/GA/GD (used to find current position)
...
// Recent league results for a club
...
// Upcoming league fixtures for a club
...
// Tournament recent results for a club
...
// Tournament upcoming fixtures for a club
...
// Match events (goals, cards, clean sheets) — League
...
// ==================================================================
// PLAYER STATS FOR A CLUB (League + Tournament combined) – FIXED
// ==================================================================
function getClubPlayersWithStats(int $club_id): array {
    global $pdo;
    ...
    $sql = "
        SELECT 
            ...
            -- League stats
            COALESCE(lg.goals, 0)      AS league_goals,
            COALESCE(lg.assists, 0)    AS league_assists,
            COALESCE(lc.yellow, 0)     AS league_yellow,
            COALESCE(lc.red, 0)        AS league_red,
            COALESCE(lcs.cs, 0)        AS league_clean_sheets,

            -- Tournament stats
            COALESCE(tg.goals, 0)      AS tournament_goals,
            COALESCE(tg.assists, 0)    AS tournament_assists,
            COALESCE(tc.yellow, 0)     AS tournament_yellow,
            COALESCE(tc.red, 0)        AS tournament_red,
            COALESCE(tcs.cs, 0)        AS tournament_clean_sheets
        ...
        -- League Clean Sheets
        LEFT JOIN (
            SELECT player_id, COUNT(*) AS cs
            FROM clean_sheets
            GROUP BY player_id
        ) lcs ON lcs.player_id = p.id

        -- Tournament Goals + Assists
        LEFT JOIN (
            SELECT 
                tg.player_id,
                COUNT(*) AS goals,
                COUNT(ta.player_id) AS assists
            FROM tournament_goals tg
            LEFT JOIN tournament_assists ta ON ta.goal_id = tg.id
        ...
        -- Tournament Clean Sheets
        LEFT JOIN (
            SELECT player_id, COUNT(*) AS cs
            FROM tournament_clean_sheets
            GROUP BY player_id
        ...
        ORDER BY 
            p.position = 'GK' ASC,
            (COALESCE(lg.goals,0) + COALESCE(tg.goals,0)) DESC,
            (COALESCE(lg.assists,0) + COALESCE(tg.assists,0)) DESC,
            p.jersey_number ASC,
            p.name ASC
    ";
...
// Add this function in properties.php (below getLeagueStandings)
function getLastFiveResults(int $clubId): array {
    ...
    // Always return exactly 5 items (pad with null if less than 5 played)
...
function getOtherPublishedArticles(int $currentId, int $limit = 6): array {
    global $pdo;
    // Fix for MariaDB/MySQL that don't allow direct integer binding in LIMIT
    $sql = "
        SELECT id, title, image, publish_date 
        FROM news 
        WHERE is_published = 1 AND id <> ? 
        ORDER BY publish_date DESC 
        LIMIT " . (int)$limit;   // ← Cast to int and concatenate safely
...
function getPlayerById(int $player_id): ?array {
    ...
    $stmt = $pdo->prepare("
        SELECT 
            ...
            p.goals, p.assists, p.yellow_cards, p.red_cards, p.clean_sheets,
...
function getResultsTotalMatchdays(int $year = null): int {
    global $pdo;
...
function getAllCompletedMatchesWithEvents(int $year = null): array {
    global $pdo;
...
/**
 * Search clubs by name (partial match)
 */
...
/**
 * Top scorers in tournaments (using players.goals column – adjust if you have a separate field)
 */
...
/**
 * Render full event timeline for a tournament match (goals, cards, clean sheets)
