...
$logoSrc = $loginLogo ? '../uploads/admin/logos/' . $loginLogo : 'includes/logo.png';
?>
...
<head>
    ...
    <title>Admin Login – 04 Soccer League</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css" rel="stylesheet">
    <style>
        body { background: linear-gradient(135deg, #0d6efd 0%, #198754 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center; margin: 0; font-family: 'Segoe UI', sans-serif; }
        .login-card { max-width: 420px; width: 100%; border-radius: 1.6rem; overflow: hidden; box-shadow: 0 25px 60px rgba(0,0,0,0.5); background: white; }
        .login-header { background: #ffffff; padding: 2.2rem 2rem 1.8rem; text-align: center; border-bottom: 6px solid #0d6efd; }
        ...
        .form-control:focus { border-color: #0d6efd; box-shadow: 0 0 0 0.25rem rgba(13, 110, 253, 0.18); }
        .btn-login { height: 54px; border-radius: 1rem; font-weight: 700; font-size: 1.12rem; background: linear-gradient(135deg, #0d6efd, #2563eb); border: none; }
        .btn-login:hover { transform: translateY(-2px); box-shadow: 0 10px 25px rgba(13, 110, 253, 0.4); }
...
<div class="login-card">
    <div class="login-header">
        ...
        <small>Secure Admin & Staff Login</small>
    </div>

    <div class="login-body">
        <?php if (isset($_GET['timeout'])): ?>
            <div class="alert alert-warning">You were logged out due to 30 minutes of inactivity.</div>
        <?php endif; ?>
...
<!-- BLOCK BACK BUTTON FROM SHOWING THIS PAGE AGAIN -->
<script>
